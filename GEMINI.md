...
- Corrected the critical path calculation algorithm in `py-scripts/generate_gantt_chart.py` to only consider tasks reachable from the `all` target, preventing "hanging tasks" from incorrectly influencing the critical path.
